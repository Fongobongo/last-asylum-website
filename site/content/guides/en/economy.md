---
title: "Economy: Diamonds, Shops, Plunder"
description: "Where every diamond goes: VIP shop priorities, the restock schedule, per-shop buy orders, Wishing Wheel budgets, and the complete plunder mechanics with daily limits by Sanctuary level."
lang: en
updated: "2026-09-03"
videoTopic: economy
infographics: ["/infographics/shop-priority.webp", "/infographics/diamonds-free.webp"]
---

Last Asylum: Plague runs three parallel economies: diamonds and shops, daily plunder, and the event calendar. Each has its own traps — bad diamond buys, wasted plunder capacity, stamina spent at 5% efficiency. This page is the community consensus on all three, with the exact numbers.

## VIP Shop: Diamond Priorities {#vip-shop}

The VIP shop takes diamonds, and most of its stock is a bad trade. The verified buy list:

| Priority | Items |
|---|---|
| HIGH | UR Omnifragment, UR Antidote, Crow Essence, 8h Speedup |
| MEDIUM | 3h Speedup, SSR Omnifragment, 1h Speedup |
| LOW | SSR Antidotes, Direct Relocation |

And the full rule: **avoid everything else unless extreme necessity.** The shop is priced to tempt impatient players into converting diamonds into items the game gives away through events — hold the line.

## Shop Restock Schedule {#restock-schedule}

All shops restock on Monday. Plan your week around it: buy out your priorities Monday when stocks are full, run your Alliance Duel and Expedition income mid-week, then spend the weekend deciding what Monday's refresh owes you. Players who shop on Sunday buy a picked-clean shelf.

## Sanctuary Shop {#sanctuary-shop}

1. Crow Essence and UR Hero Omnifragments — buy to the maximum.
2. Antidotes and SSR Omnifragments.
3. Gear Stones and Recruitment Tickets.
4. Skill Badges and SR Omnifragments — extra budget only.

## Expedition Shop {#expedition-shop}

1. UR Omnifragments and Essences.
2. Expedition Chest, Inscription Fragment, and the UR Inscription Chest.
3. Antidotes and Gear Stones.
4. Speedups, Tickets, and the 100K Crow Fruits — necessary only.

## Honor Shop {#honor-shop}

1. UR Gear Blueprint, Essence, and UR Omnifragments.
2. Curiosity Chests and UR Inscriptions.
3. Everything else — urgent needs only.

The critical rule for Honor specifically: honor is **only for gear blueprints**. There are no epigraph chests here — that is not an oversight to work around, it is how the currency is designed. Spend honor on UR gear blueprints first and treat the rest as filler.

## Alliance Shop {#alliance-shop}

1. UR Hero Omnifragment, Crow Essence, and Direct Relocation.
2. Crow Evolution Guide, Shields, SR Inscription Chest, and Bella Fragments.
3. Optional: all speedups.

## Epigraph Chests: The Exception Rule {#epigraph-chests}

Buy epigraph chests in every shop **except the Honor Shop**. Epigraphs feed the Raven system, and the Raven's UR epigraphs only buff one faction — in the mono-faction meta, that makes epigraph chests the single most consistent long-term purchase in the game.

Free-to-play epigraph income is real, if you show up for it: 30 UR shards per day from Alliance Duel, plus self-forging in the Epigraph Workshop, adds up to roughly 1.5 UR epigraphs per month. The full system — workshop levels, promotion order, and rarity rules — is covered in the [Raven guide](/raven/).

## Diamond Rules: Wheels, Banners, and Milestones {#diamond-rules}

1. **Save until day 8** for the Cynthia Wishing Wheel — she runs about 11,500 diamonds. Spin 10x only; single spins pay the same odds at a worse price.
2. **After day 15, save for Joker's wheel** — the UR PvP finisher event. The mid-game diamond reserve exists for this.
3. **Never dump diamonds into standard recruitment.** Wait for rate-up banners — same currency, better heroes.
4. **Do not chase cumulative diamond-spend milestones** like Deep Sea's Gift. Spending diamonds just to cross a reward threshold is buying the milestone, not the value.

### Best Packs ($20-200 Range) {#best-packs}

If you spend at all, the community picks three packs in the $20-200 range: Universal Speedup packs, Raven Essence packs, and City Level-Up packs. And from the beginner guides: the $1 starter pack for Marlena (who becomes free in the Tavern on Day 66) remains the single best value purchase in the game at any price.

## Plunder: The Complete Mechanics {#plunder-mechanics}

Plunder income is capped by a daily limit, and the limit scales with your Sanctuary level. Knowing the thresholds changes when you stop attacking for the day.

<!-- component:plunder-calculator -->

### How the Limit Works {#how-the-limit-works}

- The limit counts Grain, Timber, and Herbs carried home from attacks.
- Below the limit: you carry resources at **100%** efficiency.
- From 1x the limit: efficiency drops to **15%**.
- From 1.3x the limit: efficiency drops to **5%** — that is the floor.
- The limit resets at **02:00 UTC** daily.
- The yellow triangle under a target's Might is the over-limit indicator — check it via Effect Overview, then Plunder Gains.

### Daily Limits by Sanctuary Level {#plunder-limits-table}

| Sanctuary | Daily limit | Sanctuary | Daily limit | Sanctuary | Daily limit |
|---|---|---|---|---|---|
| 10-12 | 2.5M | 18 | 8M | 24 | 18M |
| 13 | 3M | 19 | 9M | 25 | 20M |
| 14 | 4M | 20 | 10M | 26 | 22M |
| 15 | 5M | 21 | 12M | 27 | 24M |
| 16 | 6M | 22 | 14M | 28 | 26M |
| 17 | 7M | 23 | 16M | 29 / 30 | 28M / 30M |

The pattern: +1M per Sanctuary level up to 20, then +2M per level to the 30M cap at Sanctuary 30. Two mechanical notes:

- A Sanctuary upgrade raises your limit **immediately** — no reset wait. If you are near the daily cap and a Sanctuary level is close, finish it first and plunder after.
- The War Supply skill from the Legion Commander specialty tree (Era of Revival) raises the cap by +6 / +14 / +24 / +36 / +50% at skill levels 1-5. See the [Era guide](/era/).

## The Nine-Hit Rule {#nine-hit-rule}

Never attack the same farm more than nine times in a row. The tenth hit destroys the city outright and teleports its owner away — you lose the farm, and the player loses their city. Nine hits is the ceiling; the community treats it as an obligation, not a suggestion.

### Field Technique: Getting a Full Farm Run {#field-technique}

Community-tested procedure for a full daily plunder session:

1. **Scout first.** Open your mail after scouting: the report shows exactly how much of each resource the target holds. Peek at 2–3 candidates and pick the richest — many targets are loaded with one resource type.
2. **Skip players in an alliance** — attacking an allied member starts wars your alliance did not sign up for.
3. **Stage on an adjacent tile.** Send your squad to a resource tile next to the target; once it's sitting there, you can launch up to ~8 rapid consecutive attacks without waiting for marches.
4. **Keep them above 2,000 city HP.** Below that they teleport away and your farmspot is gone — reuse the same farm tomorrow by stopping short.

**Soldier Load** decides how much you carry per hit. It rises **+200 per troop tier** (T6 = 1,400, T7 = 1,600, T8 = 1,800...) and can be raised further by the **Load Training** tech in the Soldier research tree. Higher-tier squads plunder the same target in fewer hits, saving stamina.

## Stamina Math: Why Over-Limit Plundering Fails {#stamina-math}

Spending stamina on over-limit plunder is one of the worst trades in the game:

- At 15% efficiency, each stamina point earns roughly **7x less** than normal.
- At 5% efficiency, roughly **20x less**.
- The same stamina spent on monsters earns **full value regardless** of your plunder status.

So the daily rhythm writes itself: hit the richest targets you can beat first while you are at 100%, stop attacking farms the moment you cross the limit, and dump the rest of your stamina into monsters. And within your plunder window, always hit the richest target first — the limit is coming regardless, so fill it with the biggest numbers available.

## Caravans and Covert Ops: Always Run Both {#caravans-covert-ops}

Caravans and Covert Ops are separate systems from plunder, and they dodge every restriction above: no plunder limit, no real troop loss, no stamina cost. They are pure upside, which is why the community rule is to run both every single day.

- **Caravans** — your automated trade income. Keep them moving.
- **Covert Ops** — you can plunder up to 5 other players' Covert Ops per day. Golden Covert Ops pay the best rewards — always grab a golden one when you see it.
